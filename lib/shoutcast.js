...
// Promise Wrapper for Async function in index.js
function _getShoutcastV1Station(url, callback) {
  return new Promise((resolve, reject) => {
    try {
      // Test for promises to see if resolve will pass through async/await functions properly
      setTimeout(() => {
        resolve({ test: url });
      }, 1000);

      //resolve(getShoutcastV1Station(url, callback));
    } catch (err) {
...
// Promise Wrapper for Async function in index.js
function _getShoutcastV2Station(url, callback) {
  return new Promise((resolve, reject) => {
    try {
      resolve(getShoutcastV2Station(url, callback));
    } catch (err) {
      reject(callback(null, err));
    }
  });
...
module.exports._getShoutcastV1Station = _getShoutcastV1Station;
module.exports._getShoutcastV2Station = _getShoutcastV2Station;
